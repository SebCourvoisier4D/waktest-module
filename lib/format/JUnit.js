exports.format = function(data) {
	...
	_report.push('<testsuites context="' + data.context + '" success="' + ((data.passes / (data.passes + data.failures)) * 100).toFixed(2) + '%" completion="' + data.completion + '%" passed="' + data.passes + '" failures="' + data.failures + '" skipped="' + data.skipped + '" tests="' + (data.passes + data.failures + data.skipped) + '" time="' + data.duration + '" path="' + util.encodedStr(data.path) + '">');
	data.suites.forEach(function(suite) {
		_report.push('<testsuite name="' + util.encodedStr(suite.title) + '" passed="' + suite.passes + '" failures="' + suite.failures + '" skipped="' + suite.skipped + '" tests="' + (suite.passes + suite.failures + suite.skipped) + '" time="' + suite.duration + '" path="' + util.encodedStr(suite.path) + '">');
		suite.tests.forEach(function(test) {
			if (test.state === 'passed') {
				...
				_report.push('<testcase name="' + util.encodedStr(test.title) + '" ignored="true" skipped="true" state="' + test.state + '">');
				_report.push('<skipped />');
				_report.push('</testcase>');
