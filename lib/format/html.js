exports.format = function(data) {
	...
	_report.push('if (typeof studio !== "undefined") studio.sendCommand("UnitTest.wakbot_any", {report: "' + require('base64').encode(JSON.stringify(data)) + '", event: "waktest_ended", kind: "ssjs", automatic: false});');
	_report.push('</script>');
	_report.push('</head>');
	...
	data.suites.forEach(function(suite) {
		...
		suite.tests.forEach(function(test) {
			if (test.state === 'passed') {
				...
				_report.push('<li class="test pending"><h2>' + util.encodedStr(test.title) + '</h2>');
				_report.push('</li>');
